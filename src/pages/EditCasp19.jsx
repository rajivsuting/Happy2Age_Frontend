...
const EditCasp19 = () => {
  ...
  const fetchEvaluation = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`/casp/${id}`);
      ...
      if (response.data.success) {
        ...
        setTotalScore(evaluation.totalScore || 0);
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.patch(`/casp/edit/${id}`, finalData);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div className="mb-4 sm:mb-0">
              <h1 className="text-2xl font-bold text-gray-900">
                Edit CASP-19 Evaluation
              </h1>
              <p className="mt-1 text-sm text-gray-500">
                Update CASP-19 evaluation details
              </p>
            </div>
