...
const OxfordHappiness = () => {
  ...
  const fetchEvaluations = async () => {
    try {
      ...
      const response = await axiosInstance.get(
        `/oxford-happiness/all?${queryParams}`
      );
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await axiosInstance.delete(
        `/oxford-happiness/delete/${selectedEvaluation._id}`
      );
      if (response.data.success) {
        fetchEvaluations();
  ...
  const getHappinessLevel = (score) => {
    if (score >= 5.5) return { level: "Very High", color: "text-green-600" };
    if (score >= 4.5) return { level: "High", color: "text-blue-600" };
    if (score >= 3.5) return { level: "Moderate", color: "text-yellow-600" };
    if (score >= 2.5) return { level: "Low", color: "text-orange-600" };
    ...
    <div className="min-h-full bg-gray-50 py-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div className="mb-4 sm:mb-0">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Manage and track Oxford Happiness evaluations
              </p>
            </div>
            <div className="flex space-x-3">
              <button
                onClick={() => navigate("/oxford-happiness/add")}
                      ...
                      <tr key={evaluation._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">
                            {evaluation.happinessScore?.toFixed(2) || "N/A"}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${happinessLevel.color} bg-opacity-10`}
                          >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() =>
                                navigate(
                                  `/oxford-happiness/edit/${evaluation._id}`
                                )
                              }
        ...
        <ConfirmationModal
          ...
          message="Are you sure you want to delete this Oxford Happiness evaluation? This action cannot be undone."
          confirmText="Delete"
          cancelText="Cancel"
          confirmColor="red"
        />
            ...
            <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-gray-800">
                  Oxford Happiness Evaluation Details
                </h3>
                <button
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">
                    {selectedEvaluation.happinessScore?.toFixed(2) || "N/A"}
                  </p>
                </div>
