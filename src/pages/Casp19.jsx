...
const Casp19 = () => {
  ...
  const fetchEvaluations = async () => {
    try {
      ...
      const response = await axiosInstance.get(`/casp/all?${queryParams}`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await axiosInstance.delete(
        `/casp/delete/${selectedEvaluation._id}`
      );
      if (response.data.success) {
        fetchEvaluations();
  ...
  const getCaspLevel = (score) => {
    if (score >= 60) return { level: "Very High", color: "text-green-600" };
    if (score >= 50) return { level: "High", color: "text-blue-600" };
    if (score >= 40) return { level: "Moderate", color: "text-yellow-600" };
    if (score >= 30) return { level: "Low", color: "text-orange-600" };
    ...
    <div className="min-h-full bg-gray-50 py-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div className="mb-4 sm:mb-0">
              <h1 className="text-2xl font-bold text-gray-900">CASP-19</h1>
              <p className="mt-1 text-sm text-gray-500">
                Manage and track CASP-19 evaluations
              </p>
            </div>
            <div className="flex space-x-3">
              <button
                onClick={() => navigate("/casp-19/add")}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {evaluations.map((evaluation) => {
                    const caspLevel = getCaspLevel(evaluation.totalScore);
                    return (
                      <tr key={evaluation._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${caspLevel.color} bg-opacity-10`}
                          >
                            {caspLevel.level}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() =>
                                navigate(`/casp-19/edit/${evaluation._id}`)
                              }
        ...
        <ConfirmationModal
          ...
          message="Are you sure you want to delete this CASP-19 evaluation? This action cannot be undone."
          confirmText="Delete"
          cancelText="Cancel"
          confirmColor="red"
        />
