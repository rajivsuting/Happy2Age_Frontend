...
const CenterDetails = () => {
  ...
  const handleDelete = async () => {
    try {
      const response = await axiosInstance.delete(`/cohort/${id}`);
      if (response.data.success) {
        navigate("/centers");
      } else {
        ...
        <div className="mb-10">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <button
                onClick={() => navigate(`/centers/edit/${id}`)}
          ...
          <div className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
            <div className="px-4 py-5 sm:p-6 border-b border-gray-200">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {center.participants?.length || 0} participants in this center
              </p>
            </div>
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {center.participants?.length > 0 ? (
                    center.participants.map((participant) => (
                      <tr
                        key={participant._id}
                        onClick={() => navigate(`/members/${participant._id}`)}
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-500">
                            {truncateText(participant.email)}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-500">
